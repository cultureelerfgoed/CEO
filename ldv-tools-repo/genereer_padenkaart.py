...
def verken(sparql: SPARQLWrapper, klasse: str) -> list[dict]:
    ...
    """)
    return [
        {k: v["value"] for k, v in rij.items()}
        for rij in sparql.query().convert()["results"]["bindings"]
    ]
...
def main() -> None:
    ...
    while wachtrij and len(bezocht) < MAX_KLASSEN:
        ...
        for pad in paden:
            ...
            if pad["soort"] == "klasse":
                g.add((ps, SH["class"], URIRef(pad["doel"])))
                if pad["doel"].startswith(CEO):
                    wachtrij.append(pad["doel"])  # transitief doorlopen
            elif pad["soort"] == "datatype":
                g.add((ps, SH.datatype, URIRef(pad["doel"])))
